# CoNET — Build Sequence: Classes & Functions, One by One

**Document 5 of the CoNET specification set.**

*The ordered list of what to code first — signatures, purpose, and internal steps. Work down this list top to bottom. Stage A and early Stage B are in full depth; later stages are outlined. Never build a unit before the ones it depends on.*

---

## 0. How to use this

Each entry is one buildable unit:
...
Guiding principle: *prototype the risky contracts as throwaways first (Stage A), then build the real vertical slice (Stage B).* Stage A code is allowed to be ugly and disposable. Stage B code is the real thing.
...
Now build the real package, in this order. Each unit lives in its proper `src/conet/` home and is production code. The target is the SRS §10 acceptance test.

### B1. Persistence layer
**File:** `src/conet/persistence/store.py` · **Depends on:** A1, A3
...
**File:** `src/conet/sdk/manifests.py` · **Depends on:** A1
...
**File:** `src/conet/control/registry.py` · **Depends on:** B1, B2
...
### B4. Discovery
**File:** `src/conet/control/discovery.py` · **Depends on:** B3, B6
...
- **Steps:**
  ...
  - For each candidate, call `PolicyEngine.authorize(requester, skill_id, 'invoke')`; drop the denied ones (FR-006).
  - Return the surviving list; empty list is a valid answer (0-provider case).

### B5. gRPC runtime
**File:** `src/conet/runtime/server.py` + `src/conet/protocols/grpc/` · **Depends on:** A2, B2
...
### B6. Policy engine
**File:** `src/conet/control/policy.py` · **Depends on:** A4, B1
...
### B7. Agent SDK
**File:** `src/conet/sdk/agent.py` · **Depends on:** B2, B3, B5
...
### B8. Event plane
**File:** `src/conet/protocols/events/bus.py` · **Depends on:** A5
...
**File:** `src/conet/observability/tracing.py` · **Depends on:** A6, B1
...
### B10. CLI
**File:** `src/conet/cli/main.py` · **Depends on:** B3, B4, B5
...
Build after v0.1 passes. Each becomes its own detailed unit when you reach it.
...
*Build top to bottom. Signatures and steps are the design; the code is yours to write. When a single unit's internals get hard, that unit is the right place to go deep.*
